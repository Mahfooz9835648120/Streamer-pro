...
export function initRoom() {
  ...
  const nameInput    = ensureNameInput(createJoin);
  const identityEl   = ensureIdentityLabel(activePanel);

  // Create room
  createBtn?.addEventListener('click', async () => {
    const name = getValidatedName(nameInput);
    if (!name) return;
...
function ensureNameInput(createJoin) {
...
function getValidatedName(nameInput) {
  const name = nameInput?.value?.trim();
  if (!name) {
    EventBus.emit(EVENTS.TOAST, { msg: 'Enter your name to continue.' });
    return '';
  }
